...
class CloudLoadBalancer(rackspace_resource.RackspaceResource):
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if 'nodes' in prop_diff:
            ...
            if new_nodes:
                logger.debug("Adding %s nodes to %s for stack %s" % 
                            (len(new_nodes), str(self), self.stack.id))
                loadbalancer.add_nodes(new_nodes)

            #Delete loadbalancers in the old dict that are not in the new dict.
            if len(deleted):
                logger.debug("Deleting %s nodes from %s for stack %s" %
                             (len(deleted), str(self), self.stack.id))
            for node in deleted:
                old[node].delete()

            #Update nodes that have been changed
            if len(updated):
                logger.debug("Updating %s nodes in %s for stack %s" %
                             (len(updated), str(self), self.stack.id))
            for node in updated:
                node_changed = False
    ...
    def handle_delete(self):
        ...
        try:
            ...
            if loadbalancer.status != 'DELETED':
                loadbalancer.delete()
                logger.debug("Deleted %s [%s] from stack %s" % (str(self), 
